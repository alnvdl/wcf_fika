...
return class Fika extends Application {
    constructor() {
    ...
    async status(ctx) {
        ...
        if (currentSessions.length == 0) {
            return new Application.Response("There are currently no fika sessions being hosted");
        }
    ...
    async start(ctx) {
        ...
        var sessions = await ctx.getData("sessions", {});
        if (user in sessions) {
            return new Application.ErrorResponse("You already have a fika session under your name. Please end it first.");
        }
        sessions[user] = {
            startDate: new Date(),
            participants: []
    ...
    async leave_list(ctx) {
        ...
        if (mySessions.length == 0) {
            return new Application.Response("You are not currently part of any fika sessions");
        }
    ...
    async join(ctx, who) {
        ...
        if (user === host) {
            return new Application.ErrorResponse(`You cannot join your own fika session`);
        }
        var sessions = await ctx.getData("sessions", {});
        var session = sessions[host];
        if (!session) {
            return new Application.ErrorResponse(`User ${host} is not currently hosting a fika session`);
        }
        if (session.participants.includes(user)) {
            return new Application.ErrorResponse(`You are already in the fika session hosted by ${host}`);
        }
        session.participants.push(user);
        await ctx.setData("sessions", sessions);

        return new Application.Response(`You joined the fika session hosted by ${host}!`);
    }
    ...
    async leave(ctx, who) {
        ...
        if (user === host) {
            return new Application.ErrorResponse(`You cannot leave your own fika session, end it instead`);
        }
        var sessions = await ctx.getData("sessions", {});
        var session = sessions[host];
        if (!session) {
        ...
        if (!session.participants.includes(user)) {
            return new Application.ErrorResponse(`You are not part of the fika session hosted by ${host}`);
        }
        session.participants.splice(session.participants.indexOf(user), 1);
        await ctx.setData("sessions", sessions);

        return new Application.Response(`You left the fika session hosted by ${host}!`);
    }
    ...
    async end(ctx) {
        ...
        if (!session) {
            return new Application.ErrorResponse(`You are not currently hosting a fika session`);
        }
        ...
        if (!ctx.getClientData("reallyEnd")) {
            ...
            return new Application.Response(`After you end a section, funds will be transferred and it will be added to the history.\n` +
            ...
            return new Application.Response(`Session ended! Your balance has increased by ${total}.\n`+
    ...
    async balances(ctx) {
        ...
        return new Application.Response(out.trim(), balances);
    }

    async history(ctx) {
        let history = await ctx.getData("history", []);
        if (history.length == 0) {
            return new Application.Response("There is no fika session history.", history);
        }
    ...
    async next(ctx) {
        ...
        return new Application.Response(out.trim(), nextList);
    }
}
